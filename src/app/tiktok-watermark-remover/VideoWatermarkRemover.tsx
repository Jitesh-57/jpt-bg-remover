    ...
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", color: "var(--text)", background: "var(--surface)" }}>
      ...
      <section style={{ background: "linear-gradient(160deg,var(--surface-2) 0%,var(--surface) 50%,var(--success-soft) 100%)", padding: "72px 24px 60px", textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: "clamp(1rem,2vw,1.15rem)", color: "var(--text-muted)", lineHeight: 1.7, maxWidth: 560, margin: "0 auto 34px" }}>
            Paste a TikTok link and download the video <strong>without the watermark</strong> in HD — free, no app, no sign-up.
          </p>

          {/* Paste box */}
          <div style={{ maxWidth: 620, margin: "0 auto", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 18, padding: 14, boxShadow: "0 12px 40px var(--accent-soft)" }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <div style={{ position: "relative", flex: "1 1 260px" }}>
                <input
                  ...
                  style={{ width: "100%", boxSizing: "border-box", padding: "14px 14px 14px 42px", borderRadius: 12, border: "1.5px solid var(--border)", fontSize: 14, outline: "none", fontFamily: "inherit" }}
                />
                <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", fontSize: 16, opacity: 0.5 }}>🔗</span>
                {!url && (
                  <button onClick={pasteFromClipboard} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "var(--surface-2)", border: "none", borderRadius: 8, padding: "6px 10px", fontSize: 12, fontWeight: 700, color: "var(--accent)", cursor: "pointer" }}>Paste</button>
                )}
              </div>
              <button
                onClick={submit}
                disabled={loading}
                style={{ ...btnBase, flex: "0 0 auto", minWidth: 170, background: GRAD, color: "#fff", boxShadow: "0 6px 20px rgba(255,106,26,0.40)", opacity: loading ? 0.7 : 1 }}
              >
            ...
            {error && (
              <div style={{ marginTop: 10, background: "var(--danger-soft)", border: "1px solid var(--danger-soft)", borderRadius: 10, padding: "9px 12px", fontSize: 13, color: "#C0392B", textAlign: "left" }}>{error}</div>
            )}
            <div style={{ marginTop: 10, fontSize: 12, color: "var(--text-faint)" }}>Works with tiktok.com, vm.tiktok.com & vt.tiktok.com links.</div>
          </div>

          {/* Result */}
          {result && (
            <div style={{ maxWidth: 620, margin: "24px auto 0", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 18, padding: 16, boxShadow: "0 8px 30px rgba(0,0,0,0.06)", textAlign: "left" }}>
              <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
                {result.cover && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={result.cover} alt={result.title} style={{ width: 92, height: 122, objectFit: "cover", borderRadius: 12, flexShrink: 0, background: "var(--surface-2)" }} />
                )}
                <div style={{ flex: "1 1 220px", minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: "var(--text)", marginBottom: 4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden" }}>{result.title}</div>
                  <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 14 }}>
                    {result.author && <>@{result.author}</>}{result.duration ? <> · {fmtDur(result.duration)}</> : null}
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <a href={dl(result.noWatermark, "video")} style={{ ...btnBase, background: GRAD, color: "#fff", boxShadow: "0 4px 16px rgba(255,106,26,0.40)" }}>⬇ Download (No Watermark)</a>
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      {result.music && (
                        <a href={dl(result.music, "audio")} style={{ ...btnBase, flex: 1, background: "var(--surface-2)", color: "var(--text)" }}>🎵 Audio (MP3)</a>
                      )}
                    </div>
                  </div>
                </div>
          ...
          {HERO_IMG && !result && (
            <div style={{ maxWidth: 640, margin: "40px auto 0" }}>
              <div style={{ position: "relative", borderRadius: 22, padding: 8, background: "linear-gradient(135deg,var(--accent-soft),var(--accent-soft))", boxShadow: "0 24px 60px var(--accent-soft)" }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={HERO_IMG}
      ...
      <section style={{ padding: "72px 24px", background: "var(--surface)" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: "clamp(1.8rem,3vw,2.3rem)", fontWeight: 900, color: "var(--text)", margin: 0, letterSpacing: "-0.02em" }}>Three steps, no watermark</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 28 }}>
            {[
              ...
              <div key={s.n} style={{ textAlign: "center" }}>
                <div style={{ width: 66, height: 66, background: "var(--surface)", border: "2px solid var(--accent-soft)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px", boxShadow: "0 4px 20px var(--accent-soft)", fontSize: 20, fontWeight: 900, color: "var(--accent)" }}>{s.n}</div>
      ...
      {/* FEATURES / trust */}
      <section style={{ padding: "0 24px 40px", background: "var(--surface)" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
          {[
            ...
            <div key={c.t} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 999, padding: "9px 16px" }}>
              <span style={{ fontSize: 15 }}>{c.i}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-muted)" }}>{c.t}</span>
            </div>
          ))}
        </div>
        <p style={{ maxWidth: 720, margin: "22px auto 0", textAlign: "center", fontSize: 12.5, color: "var(--text-faint)", lineHeight: 1.6 }}>
      ...
      {/* WHY BEST */}
      <section style={{ padding: "72px 24px", background: "linear-gradient(135deg,var(--surface-3),var(--bg-elevated))" }}>
      ...
      {/* FAQ */}
      <section style={{ padding: "40px 24px 88px", background: "var(--surface-2)" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            <h2 style={{ fontSize: "clamp(1.7rem,3vw,2.1rem)", fontWeight: 900, color: "var(--text)", margin: 0, letterSpacing: "-0.02em" }}>Frequently asked questions</h2>
          </div>
          {[

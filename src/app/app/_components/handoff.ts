 ...
 */
export function sendToCreate(prompt: string, navigate: (href: string) => void, opts?: { needsPhoto?: boolean; reference?: string | null }): void {
  try { sessionStorage.setItem(CREATE_PROMPT_KEY, JSON.stringify({ prompt, needsPhoto: !!opts?.needsPhoto, reference: opts?.reference ?? null } satisfies CreateHandoff)); } catch {}
  navigate("/app/create");
}

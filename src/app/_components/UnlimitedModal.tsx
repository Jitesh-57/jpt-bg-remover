...
export default function UnlimitedModal({ onClose, loggedIn, reason, prefillUser, onSuccess }: Props) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, backdropFilter: "blur(4px)" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--surface)", borderRadius: 22, maxWidth: 420, width: "100%", boxShadow: "0 24px 80px rgba(0,0,0,0.35)", overflow: "hidden" }}>
        {/* header */}
        <div style={{ background: "linear-gradient(135deg,var(--accent),var(--accent-2))", padding: "30px 30px 26px", color: "#fff", position: "relative", textAlign: "center" }}>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 13.5, color: "rgba(12,12,22,0.80)" }}>
        ...
        <div style={{ padding: "26px 30px 30px" }}>
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            <span style={{ fontSize: 44, fontWeight: 900, color: "var(--text)", letterSpacing: "-0.03em" }}>{UNLIMITED_PRICE_LABEL}</span>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 2 }}>{UNLIMITED_PRICE_SUB}</div>
          </div>

          <ul style={{ listStyle: "none", margin: "0 0 22px", padding: 0, display: "flex", flexDirection: "column", gap: 10 }}>
            {FEATURES.map((f) => (
              <li key={f} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: "var(--text-muted)", fontWeight: 600 }}>
                <span style={{ width: 20, height: 20, borderRadius: "50%", background: "var(--success-soft)", color: "var(--success)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 900, flexShrink: 0 }}>✓</span>
                {f}
              </li>
            ))}
          </ul>
          ...
          {statusMsg && (
            <div style={{ marginBottom: 16, padding: "11px 14px", borderRadius: 10, background: statusMsg.ok ? "var(--success-soft)" : "var(--danger-soft)", color: statusMsg.ok ? "var(--success)" : "var(--danger)", fontSize: 13.5, fontWeight: 700, textAlign: "center" }}>
              {statusMsg.text}
            </div>
          )}
          ...
          {loggedIn ? (
            ...
            <>
              <button
                ...
                className="jpt-hover"
                style={{ width: "100%", padding: "14px", background: "var(--surface)", color: "var(--text-muted)", border: "1.5px solid var(--border)", borderRadius: 12, fontWeight: 800, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}
              >

...
const FREE_IMAGE_TOOLS = [
  ...
  { icon: "↔️", label: "Resize Image",    desc: "Resize to exact pixels",        href: "/resize-image", free: true },
  { icon: "🔄", label: "Rotate & Flip",   desc: "Turn or mirror photos",         href: "/rotate-image", free: true },
  { icon: "🔳", label: "QR Code Generator", desc: "Link or text to QR code",     href: "/qr-code-generator", free: true },
  { icon: "🫥", label: "Blur Image",      desc: "Hide faces & sensitive info",   href: "/blur-image", free: true },
  ...
  { icon: "🪄", label: "Watermark Remover", desc: "Erase watermarks from photos", href: "/watermark-remover", free: true },
];
...
export default function NavBar() {
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      ...
      // Reload the page they signed in from rather than sending them elsewhere.
        ...
        <div style={{ maxWidth: 1200, width: "100%", margin: "0 auto", display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
          ...
          */}
          ...
          {PAID_FEATURES_ENABLED && (
            <a href="/creative"
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", color: "var(--text-muted)", fontSize: 14, fontWeight: 600, textDecoration: "none", borderRadius: 8, background: "var(--accent-soft)", border: "1px solid var(--accent-border)" }}
              onMouseEnter={e => { e.currentTarget.style.background = "var(--accent-border)"; e.currentTarget.style.borderColor = "rgba(255,106,26,0.40)"; }}
          ...
          {user ? (
            <div style={{ position: "relative" }}>
              <button onClick={() => setShowMenu(!showMenu)}
                style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px 6px 6px", background: "var(--accent-soft)", border: "1px solid rgba(255,106,26,0.40)", borderRadius: 24, cursor: "pointer" }}>
                {user.picture

...
const INPUT: React.CSSProperties = { border: "1.5px solid var(--border)", borderRadius: 10, padding: "12px 13px", fontSize: 14, fontFamily: "inherit", outline: "none", width: "100%", boxSizing: "border-box" };
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(15,23,42,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", width: "100%", maxWidth: 400, background: "var(--surface)", borderRadius: 20, boxShadow: "0 24px 70px rgba(0,0,0,0.35)", padding: "30px 28px 24px" }}>
        ...
        <p style={{ fontSize: 14, color: "var(--text-muted)", lineHeight: 1.55, margin: "0 0 20px" }}>{sub}</p>
        ...
        <ul style={{ listStyle: "none", margin: "0 0 22px", padding: 0, display: "flex", flexDirection: "column", gap: 9 }}>
          {["Free basic image tools", "Your edits saved to your account", "No credit card required"].map((f) => (
            <li key={f} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13.5, color: "var(--text-muted)", fontWeight: 600 }}>
              <span style={{ width: 18, height: 18, borderRadius: "50%", background: "var(--success-soft)", color: "var(--success)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 900, flexShrink: 0 }}>✓</span>
              {f}
            </li>
          ))}
        </ul>
        ...
        {!showEmail ? (
          <button onClick={() => setShowEmail(true)} style={{ display: "block", width: "100%", marginTop: 12, padding: "12px", borderRadius: 12, border: "1.5px solid var(--border)", background: "var(--surface)", color: "var(--text-muted)", fontSize: 14.5, fontWeight: 700, cursor: "pointer" }}>
            Continue with email
          </button>
        ) : (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 9 }}>
            <div style={{ display: "flex", gap: 4, justifyContent: "center", fontSize: 13 }}>
              <button onClick={() => { setMode("signup"); setError(""); }} style={{ background: "none", border: "none", cursor: "pointer", fontWeight: mode === "signup" ? 800 : 500, color: mode === "signup" ? "var(--accent)" : "var(--text-faint)", borderBottom: mode === "signup" ? "2px solid var(--accent)" : "2px solid transparent", padding: "4px 12px" }}>Create account</button>
              <button onClick={() => { setMode("login"); setError(""); }} style={{ background: "none", border: "none", cursor: "pointer", fontWeight: mode === "login" ? 800 : 500, color: mode === "login" ? "var(--accent)" : "var(--text-faint)", borderBottom: mode === "login" ? "2px solid var(--accent)" : "2px solid transparent", padding: "4px 12px" }}>Sign in</button>
            </div>
            ...
            {error && <div style={{ background: "var(--danger-soft)", border: "1px solid var(--danger-soft)", borderRadius: 8, padding: "8px 12px", fontSize: 12.5, color: "#C0392B" }}>{error}</div>}
            <button onClick={emailAuth} disabled={loading} style={{ padding: "12px", borderRadius: 10, border: "none", background: loading ? "var(--text-faint)" : GRAD, color: "#fff", fontSize: 14.5, fontWeight: 800, cursor: loading ? "not-allowed" : "pointer" }}>
              {loading ? "Please wait…" : mode === "signup" ? "Create account — free" : "Sign in"}
            </button>
          </div>
        )}

        <button onClick={onClose} style={{ display: "block", margin: "16px auto 0", background: "none", border: "none", color: "var(--text-faint)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Maybe later
        </button>
      </div>
    </div>

  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "system-ui,-apple-system,sans-serif", color: "var(--text)", background: "var(--surface)" }}>
        ...
        <section style={{ background: "linear-gradient(160deg,var(--surface-2) 0%,var(--surface) 55%,var(--success-soft) 100%)", padding: "64px 24px 52px", textAlign: "center" }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 18 }}>
              ...
              {" / "}
              <Link href="/alternatives" style={{ color: "var(--text-muted)", textDecoration: "none" }}>Alternatives</Link>
              {" / "}
              <span style={{ color: "var(--text-muted)" }}>{ct.name}</span>
            </div>
            ...
            <Link href={ct.tool.href} className="jpt-hover" style={{ display: "inline-block", background: GRAD, color: "#fff", borderRadius: 12, padding: "15px 34px", fontSize: 16, fontWeight: 800, textDecoration: "none", boxShadow: "0 8px 24px rgba(255,106,26,0.40)" }}>
        ...
        {/* INTRO + WHY SWITCH */}
        <section style={{ padding: "56px 24px 8px", background: "var(--surface)" }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            <p style={{ fontSize: 16.5, color: "var(--text-muted)", lineHeight: 1.8, margin: "0 0 32px" }}>{ct.intro}</p>
            <h2 style={{ fontSize: "clamp(1.5rem,3vw,2rem)", fontWeight: 800, color: "var(--text)", margin: "0 0 18px", letterSpacing: "-0.02em" }}>{ct.whyHeading}</h2>
            <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "grid", gap: 12 }}>
              {ct.gripes.map((g) => (
                <li key={g} style={{ display: "flex", gap: 12, alignItems: "flex-start", fontSize: 15.5, color: "var(--text-muted)", lineHeight: 1.65 }}>
                  <span aria-hidden style={{ flex: "none", width: 22, height: 22, borderRadius: "50%", background: "var(--danger-soft)", color: "var(--danger)", fontWeight: 900, fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 1 }}>✕</span>
                  <span>{g}</span>
                </li>
              ))}
            </ul>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14.5, minWidth: 460 }}>
                ...
                <tbody>
                  {ct.compareRows.map((r, i) => (
                    <tr key={r.feature} style={{ borderTop: "1px solid var(--border)", background: i % 2 ? "var(--surface-3)" : "var(--surface-2)" }}>
                      <td style={{ padding: "13px 16px", color: "var(--text-muted)", fontWeight: 600 }}>{r.feature}</td>
                      <td style={{ padding: "13px 16px", color: "var(--success)", fontWeight: 800 }}>{r.us}</td>
                      <td style={{ padding: "13px 16px", color: "var(--text-muted)" }}>{r.them}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p style={{ fontSize: 12.5, color: "var(--text-faint)", margin: "12px 2px 0", lineHeight: 1.5 }}>
        ...
        {related.length > 0 && (
          <section style={{ padding: "48px 24px 72px", background: "var(--surface)" }}>
            <div style={{ maxWidth: 900, margin: "0 auto" }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: "var(--text)", margin: "0 0 20px", letterSpacing: "-0.02em" }}>More free alternatives</h2>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {related.map((r) => (
                  <Link key={r.slug} href={`/alternatives/${r.slug}`} className="jpt-hover" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 999, padding: "9px 16px", fontSize: 14, fontWeight: 600, color: "var(--text)", textDecoration: "none" }}>
                    {r.name} alternative
                  </Link>
                ))}
                <Link href="/tools" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 999, padding: "9px 16px", fontSize: 14, fontWeight: 600, color: "var(--text)", textDecoration: "none" }}>
                  All free tools
                </Link>
              </div>
            </div>

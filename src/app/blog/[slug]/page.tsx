...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <main style={{ background: "var(--surface-2)", minHeight: "100vh", padding: "48px 24px 80px" }}>
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 13, color: "var(--text-faint)", marginBottom: 28 }}>
            ...
            {" / "}
            <Link href="/blog" style={{ color: "var(--text-faint)", textDecoration: "none" }}>Blog</Link>
            {" / "}
          ...
          <article>
            ...
            <h1 style={{ margin: "0 0 18px", fontSize: "clamp(26px, 4vw, 38px)", fontWeight: 900, color: "var(--text)", lineHeight: 1.18, letterSpacing: "-0.6px" }}>
              {post.title}
            </h1>
          ...
          {/* CTA section */}
          <div style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-2))", borderRadius: 20, padding: "40px 36px", textAlign: "center" as const, marginTop: 20 }}>
            ...
            <Link href={post.toolHref} style={{ display: "inline-block", background: "var(--surface)", color: "var(--accent)", borderRadius: 12, padding: "13px 28px", fontSize: 15, fontWeight: 800, textDecoration: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}>
              {post.toolLabel}
            </Link>
          </div>
          ...
          <div style={{ marginTop: 56 }}>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: "0 0 20px", letterSpacing: "-0.3px" }}>
              More free tools
            </h2>
            <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 12 }}>
              {FREE_TOOL_LINKS.filter((tool) => tool.href !== post.toolHref).map((tool) => (
                <Link
                  key={tool.href}
                  href={tool.href}
                  style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 999, padding: "9px 16px", fontSize: 14, fontWeight: 600, color: "var(--text)", textDecoration: "none" }}
                >
          ...
          {related.length > 0 && (
            <div style={{ marginTop: 56 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: "0 0 24px", letterSpacing: "-0.3px" }}>
                Related articles
              </h2>
                ...
                {related.map((r) => (
                  <Link key={r.slug} href={`/blog/${r.slug}`} style={{ textDecoration: "none", display: "block", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "14px 16px" }}>
                      ...
                      <div style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 8 }}>{r.readTime}</div>
                    </div>
                  </Link>
                ))}
              </div>

  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "system-ui,-apple-system,sans-serif", color: "var(--text)", background: "var(--surface)" }}>
        <section style={{ background: "linear-gradient(160deg,#F0F9FF 0%,var(--surface) 55%,var(--surface-2) 100%)", padding: "64px 24px 52px", textAlign: "center" }}>
          <div style={{ maxWidth: 720, margin: "0 auto" }}>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 18 }}>
              <Link href="/" style={{ color: "var(--text-muted)", textDecoration: "none" }}>Home</Link>{" / "}
              <Link href="/compress-image" style={{ color: "var(--text-muted)", textDecoration: "none" }}>Image Compressor</Link>{" / "}
              <span style={{ color: "var(--text-muted)" }}>{ct.label}</span>
            </div>
            ...
            <SafeImage
              ...
              style={{ width: "100%", height: "auto", display: "block", borderRadius: 16, border: "1px solid var(--border)", boxShadow: "0 18px 50px rgba(14,165,233,0.14)" }}
            />
          </div>
        </section>
        ...
        <section style={{ padding: "56px 24px", background: "var(--surface)" }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            <p style={{ fontSize: 16.5, color: "var(--text-muted)", lineHeight: 1.8, margin: "0 0 28px" }}>{ct.intro}</p>
            <h2 style={{ fontSize: "clamp(1.5rem,3vw,2rem)", fontWeight: 800, color: "var(--text)", margin: "0 0 16px", letterSpacing: "-0.02em" }}>Why compress to {ct.label}?</h2>
        ...
        <section style={{ padding: "8px 24px 56px", background: "var(--surface)" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            <h2 style={{ fontSize: "clamp(1.5rem,3vw,2rem)", fontWeight: 800, color: "var(--text)", margin: "0 0 32px", letterSpacing: "-0.02em", textAlign: "center" }}>How to compress an image to {ct.label}</h2>
        ...
        <section style={{ padding: "48px 24px 72px", background: "var(--surface)" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "var(--text)", margin: "0 0 20px", letterSpacing: "-0.02em" }}>Compress to other sizes</h2>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
              {related.map((r) => (
                <Link key={r.slug} href={`/compress/${r.slug}`} className="jpt-hover" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 999, padding: "9px 16px", fontSize: 14, fontWeight: 600, color: "var(--text)", textDecoration: "none" }}>
                  Compress to {r.label}
                </Link>
              ))}
            </div>
